 ...
 *
 */
public class EquipmentLockHandler implements IGameHandler {
    ...
    public void run(JSONObject request, JSONObject response, User user) {
        if(Tools.reqParamCheck(request, "i")){
            ...
            module.lockEquip(request.getInteger("i"));
        }else{
            throw new ClientException(ErrorCode.PARAMETER_ERROR, "EquipmentLockHandler缺少必要参数,传入参数:" + request.toString());
        }
    }
}
